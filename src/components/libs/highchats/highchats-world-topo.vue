...
export default defineComponent({
	name: 'highcharts-world-map',
	components: {Chart},
	setup: function () {

		// example
		// https://www.highcharts.com/demo/maps/topojson-projection
...
// Add flight route after initial animation
		const afterAnimate = (e: { target: { chart: any; }; }) => {
			const chart = e.target.chart;

			if (!chart.get('flight-route')) {
				chart.addSeries({
					type: 'mapline',
					name: 'Flight route, Moscow - Los Angeles',
					animation: false,
					id: 'flight-route',
					data: [{
						geometry: {
				...
				chart.addSeries({
					...
					data: [{
						...
						geometry: {
							type: 'Point',
							coordinates: [37.36, 55.44]
						}
					}, {
						name: 'Yekat',
						geometry: {
							type: 'Point',
							coordinates: [60.35, 56.50] // долгота, широта
						}
					}],
					color: '#313f77',
					accessibility: {
